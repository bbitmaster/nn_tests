...
if(len(sys.argv) > 1):
    ...
    params_file = 'mnist_train_params.py'
    
p = {}
execfile(params_file,p);

images, labels = read_mnist(range(10),"training",path=p['data_dir']);
...
test_images, test_labels = read_mnist(range(10),"testing",path=p['data_dir']);
test_labels = test_labels.transpose()[0] #put labels in an array
...
test_data = test_images.reshape(test_size,28*28)

test_class = np.zeros((test_size,10))
for i in range(test_size):
    test_class[i,test_labels[i]] = 1.0;
