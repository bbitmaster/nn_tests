...
cache_fname=None
if(p.has_key('cache_dir') and p['cache_dir'] is not None):
    cache_fname = p['cache_dir'] + 'cifar_data_cache_' + str(p.get('skip_pca',0)) + \
        '_' + str(p.get('reduce_to',3072)) + '.h5py'
reduce_to = p.get('reduce_to',3072)

#load the cache if we have it
if(cache_fname is not None and os.path.isfile(cache_fname)):
    print("loading cache file: " + str(cache_fname))
    try:
        f_handle = h5py.File(cache_fname,'r')
    except IOError:
        print("Error Opening: " + str(filename))
        raise IOError
    data_reduced = np.array(f_handle['data_reduced'])
    test_data_reduced = np.array(f_handle['test_data_reduced'])
    class_data = np.array(f_handle['class_data'])
    test_class_data = np.array(f_handle['test_class_data'])
    f_handle.close();    
else:
    (data_full,class_data,test_data_full,test_class_data) = load_cifar10(p['correct_target'],p['incorrect_target'])
    ...
    #save cached pca data, if applicable
    if(p.has_key('cache_dir') and p['cache_dir'] is not None):
        print("saving cache file: " + str(cache_fname))
        try:
            f_handle = h5py.File(cache_fname,'w')
        except IOError:
            print("Error Opening: " + str(filename))
            raise IOError
        f_handle['data_reduced'] = data_reduced
        f_handle['test_data_reduced'] = test_data_reduced
        f_handle['class_data'] = class_data
        f_handle['test_class_data'] = test_class_data
        f_handle.close();    
